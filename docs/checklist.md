...
These are the biggest gaps between the current prototype and the intended direction.
...
- `validate` and `export` commands now exist as tooling foundations
...
- make `src/syntax.js` the single source of truth for the public language
...
1. Refactor [syntax.js](C:\Users\joshr\Documents\dev\Brackets\src\syntax.js) into a centralized syntax contract with Datastar-first mappings.
2. Refactor [runtime.js](C:\Users\joshr\Documents\dev\Brackets\src\runtime\runtime.js) so Datastar owns the reactive binding path.
3. Tighten [server.js](C:\Users\joshr\Documents\dev\Brackets\src\server.js) around allowlists, request limits, and a smaller framework boundary.
