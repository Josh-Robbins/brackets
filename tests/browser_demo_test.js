...
Deno.test({
  name: 'browser smoke: demo with cspNonceInlineScripts has no CSP console violations',
  sanitizeOps: false,
  sanitizeResources: false
}, async () => {
  const instance = await createServer({
  ...
  try {
    ...
    const cspRelated = consoleErrors.filter(
      (line) => /content security policy|csp violation|violates the following content security policy/i.test(line)
    );
    assert(
      cspRelated.length === 0,
      `Expected no CSP-related console errors, got: ${cspRelated.join(' | ')}`
    );
    assert(
      consoleErrors.length === 0,
      `Expected no console/page errors, got: ${consoleErrors.join(' | ')}`
